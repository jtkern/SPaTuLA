...
dicty_modeling <- function(n, ncell, dat, dc, k, D, t_w, t_total, dt, nframe){
  # n: number of grid points in each dimension
  # ncell: number of cells
  ...
  for (t_ind in seq_len(nt_all)) {
    ...
    dcdt = D * (c_x_plus_one + c_x_minus_one + c_y_plus_one + c_y_minus_one) - k * con
    ...
    for (i in seq_len(ncell)){
      ...
      # }
      if(ind_y == 1) { # first column maintains morphogen gradient (L-->R)
...
rho = 1  # cell density
mean_threshold = 2 # threshold concentration
dc = 5 # cAMP production during excited state
k = .5 # degradation rate # 100=works!
D = 0.11 # diffusion constant (???)
...
dat_init = init_cells(n, ncell, states, mean_threshold)
...
plot(matrix(dat_init$con, nrow=n), key = NULL, xlab = "", ylab = "", 
     axis.col = NULL, axis.row = NULL, main = "conc.", cex=0.6)
plot(matrix(dat_init$states, nrow=n), key = NULL, xlab = "", ylab = "", 
     axis.col = NULL, axis.row = NULL, main = "cell states", cex=0.6)
plot(matrix(dat_init$cell_t, nrow=n), key = NULL, xlab = "", ylab = "", 
     axis.col = NULL, axis.row = NULL, main = "cell_threshold", cex=0.6)

# modeling
results = dicty_modeling(n, ncell, dat_init, dc, k, D, t_w, t_total, dt, nframe)
...
anim_save("~/KaitlynRRStudio/BIOE_Capstone/cell_twostates_switching_n10_sp.gif")
...
anim_save("~/KaitlynRRStudio/BIOE_Capstone/cell_twostates_conc_sp.gif")
